    ...
    <div class="main-panel">
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title ">SERVICE MANAGEMENT TABLE</h4>
                </div>
                <div class="card-body">
                  <table id="productTable" class="stripe" style="width:100%">
                    ...
                    <tbody>
                      <?php
                      $table_service_catalog = mysqli_query($con, "select s.id, s.name as productName, price, time, c.name as typeName  from service s, catalog_service c where s.catalogServiceId = c.id;");
                      while ($row = mysqli_fetch_assoc($table_service_catalog)) {
                        $table_service_image = mysqli_query($con, "select * FROM services_images where serviceId=" . $row['id'] . " and avatar=1");
                        if ($table_service_image->num_rows == 0) {
                          $table_service_image = mysqli_query($con, "select * FROM services_images where serviceId=" . $row['id']);
                        }
                        ?>
                        <tr id="rowTable<?= $row['id'] ?>">
                          <td><?php while ($row_images = mysqli_fetch_assoc($table_service_image)) {
                                ?>
                          ...
                          <td><?= $row['typeName'] ?></td>
                          <td><?= $row['price'] ?></td>
                          <td><?= (intval($row['time']) != 0) ? intval($row['time']) . "h :" : "" ?> <?= ($row['time'] - intval($row['time'])) != 0 ? ((($row['time'] - intval($row['time'])) * 60) . "m") : "00m"  ?></td>
                          <td><a href="" data-toggle="modal" data-target=".edit-form" class="edit-button" id="service<?= $row['id'] ?>"><i style="font-size:25px;color:rgb(255, 136, 0)" class="fa fa-edit"></i></a></td>
      ...
      <div class="modal fade edit-form" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <form action="editService.php" enctype="multipart/form-data" id="edit-form" method="POST">
              <legend>SERVICES EDITER FORM</legend>
              <div class="container-fluid">
                <div class="row">
                  ...
                  <div class="col-md-8" style="display:none;">
                    <label>id</label>
                    <input name="idService" id="idService" type="text" value="" class="form-control">
                  </div>
                  <div class="col-md-12">
                    <label>Name</label>
                  ...
                  <div class="col-md-3">
                    <label>Price</label>
                    <input id="price-textbox" min="1000" max="10000000" name="price" type="number" min=1000 class="form-control" required="required">
                  </div>
                  <div class="col-md-2">
                    <select style="margin-top:30px;" id="type" name="typeService" class="browser-default custom-select">
                  ...
                  <div class="col-md-7" id="time-group" style="margin-top:10px">
                    <div class="label-time">
                      <b>Time:</b>
                    </div>
                    <div class="form-group" id="hours" style="margin-top:10px">
                      
                      <label for="">Hours</label>
                      <input style="margin-top:10px" min="0" max="5"  id="hours-text" type="number" step=1 name="hours" class="form-control">
                    </div>
                    <b>|</b>
                    <div class="form-group" id="minutes">
                    
                      <label for="">Minutes</label>
                      <input style="margin-top:10px" min="0" max="60" id="minutes-text" type="number" step=15 min="0" max="60" name="minutes" class="form-control">
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="container-fluid">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    ...
                    <input type="file" name="file[]" class="filestyle" data-classButton="btn-white" data-input="true" data-classIcon="icon-plus" multiple>
                  </div>
                </div>
              </div>
      ...
      <div class="modal fade delete-alert" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md">
          <div class="modal-content">
            <h2 style="text-align:center; margin:20px;auto">Are you sure to delete this service?</h2>
      ...
      $('body').on("click",".edit-button",function() {
        ...
        $.ajax({
          ...
          success: function(e) {
            ...
            if (e.catalogServiceId == 1) {
              $('select option:nth-child(2)').prop("selected", "selected");
            }
            if (e.catalogServiceId == 2) {
              $('select option:nth-child(3)').prop("selected", "selected");
            }
            if (e.catalogServiceId == 3) {
              $('select option:nth-child(4)').prop("selected", "selected");
            }
            if (e.catalogServiceId == 4) {
              $('select option:nth-child(5)').prop("selected", "selected");
            }
            var h = parseInt(e.time);
            var m = (e.time - parseInt(e.time));
            $('#hours-text').val(h);
            $("#minutes-text").val(m * 60);
