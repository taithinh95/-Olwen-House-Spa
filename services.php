...
<body>
                ...
                <form>
                    ...
                    if (isset($_SESSION['current_user'])) {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            <div class="form-group register-form-group">
                                <input disabled="disabled" class="form-control" value="Status: <?= $row['status'] ?>" type="text">
                            </div>
    ...
    <!-- /projects -->
    <?php
    $number_record = mysqli_query($con, "select * from service")->num_rows;
    $number_page = ceil($number_record / 9);
    $current_page = $_GET['page'];
    $limit = ($current_page - 1) * 9;
    $resultProduct = mysqli_query($con, 'select * from service limit ' . $limit . ',9');
    ...
    <section class="projects py-5" id="gallery">
        <div class="container py-md-5">
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <div id="carousel<?php echo $row['id'] ?>" class="carousel slide" data-ride="carousel">
                                                <div class="carousel-inner">
                                                    <?php $result_images_slide = mysqli_query($con, "select * from services_images where serviceId=" . $row['id'] . " and avatar=1 order by avatar desc ");
                                                    if ($result_images_slide->num_rows == 0) {
                                                        $result_images_slide = mysqli_query($con, "select * from services_images where serviceId=" . $row['id'] . " order by avatar desc limit 1");
                                                    }
                                                    ...
                                                    <?php } ?>
                                                    <?php $result_images_slide = mysqli_query($con, "select * from services_images where serviceId=" . $row['id'] . " order by avatar desc  limit 1,100");
                                                    while ($row_images = mysqli_fetch_assoc($result_images_slide)) { ?>
            ...
            <div class="pagination" style="display:block;text-align:center">
                <a href="services.php?page=1"><button><i class="fa fa-step-backward"></i></button></a>
                <?php

                for ($i = 1; $i <= $number_page; $i++) {
                    ?>
                    <a href="services.php?page=<?= $i ?>"><button><?= $i ?></button></a>
                <?php
                } ?>
                <a href="services.php?page=<?= $number_page ?>"><button><i class="fa fa-step-forward"></i></button></a>
